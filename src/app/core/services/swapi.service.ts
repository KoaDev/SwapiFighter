...
export class SwapiService {
  ...
  getRandomCharacter(): Observable<Character> {
    ...
    return this.http.get<Character>(`${this.apiUrl}/people/${randomId}/`).pipe(
      map(character => this.enrichCharacterData(character))
    );
  }
  ...
  private enrichCharacterData(character: Character): Character {
    ...
    const height = character.height === 'unknown' ? 150 : parseInt(character.height);
    ...
    // Bonus équipement
    const equipmentBonus = character.starships.length * 15 + character.vehicles.length * 10;
  ...
  private isForceSensitive(name: string): boolean {
    const forceUsers = ['Skywalker', 'Vader', 'Yoda', 'Palpatine', 'Maul', 'Dooku'];
